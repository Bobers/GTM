...
const GTMSlotMachine = () => {
  ...
  const productStages = [
    { id: 'idea', name: 'Idea Stage', description: 'Just an concept, no code yet' },
    { id: 'mvp', name: 'MVP/Prototype', description: 'Basic version built' },
    { id: 'beta', name: 'Beta/Early Access', description: 'Testing with users' },
    { id: 'launched', name: 'Launched', description: 'Live with paying customers' },
    { id: 'scaling', name: 'Scaling', description: 'Growing rapidly' }
  ];
  ...
  const handleProductSubmit = (e: React.FormEvent) => {
    ...
    if (productInfo.name && productInfo.category && productInfo.stage) {
      ...
      // Customize initial slots based on product info
      // This is where you could adjust variations based on category/stage
    }
  };
  ...
  // Show product onboarding if product not defined or editing
          ...
          <form onSubmit={handleProductSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g., InventoryPro, TaskMaster, DataSync"
                required
              />
            </div>
              ...
              <div className="grid grid-cols-3 md:grid-cols-4 gap-3">
                {productCategories.map((cat) => (
                  <button
                    key={cat.id}
                    type="button"
                    onClick={() => setProductInfo({ ...productInfo, category: cat.id })}
                    className={`p-3 rounded-lg border-2 transition-all ${
                      productInfo.category === cat.id
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                rows={3}
                placeholder="What problem does your product solve? Who is it for?"
              />
            </div>

            {/* Stage Selection - Only show after category is selected */}
                ...
                <div className="space-y-2">
                  {productStages.map((stage) => (
                    <button
                      key={stage.id}
                      type="button"
                      onClick={() => setProductInfo({ ...productInfo, stage: stage.id })}
                      className={`w-full p-3 rounded-lg border-2 text-left transition-all ${
                        productInfo.stage === stage.id
                          ? 'border-purple-500 bg-purple-500/20'
                          : 'border-gray-700 hover:border-gray-600'
                      }`}
                    >
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-2xl">
                {productCategories.find(c => c.id === productInfo.category)?.icon}
              </span>
              <h2 className="text-lg font-semibold">{productInfo.name}</h2>
            </div>
            <div className="text-xs text-gray-400">
              {productCategories.find(c => c.id === productInfo.category)?.name} • {' '}
              {productStages.find(s => s.id === productInfo.stage)?.name}
              {productInfo.description && (
                <span className="block mt-1">{productInfo.description}</span>
              )}
            </div>
          </div>
          <button
